...
# Get default output path from the scene settings
scene = bpy.context.scene
output_directory = bpy.path.abspath(scene.render.filepath)

# Generate filename with YYMMDD format
date_suffix = datetime.datetime.now().strftime("%y%m%d")
export_filename = f"rotation-data-{date_suffix}.csv"
export_path = os.path.join(output_directory, export_filename)

export_rotation_data(export_path)